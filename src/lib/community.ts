...
export const POST_SELECT =
  "id, title, body, category_id, category_slug, category_name, category_emoji, image_url, is_anonymous, is_demo, score, comment_count, created_at, author_id, author_username, author_avatar_url, author_verified, is_owner";
...
export async function fetchPosts(opts: {
  ...
  if (opts.search && opts.search.trim()) {
    // PostgREST parses commas, parentheses, quotes and wildcard characters in
    // .or() expressions. Quote and escape the value before interpolating it.
    const term = opts.search.trim().replace(/[\\%_(),"]/g, (char) => `\\${char}`);
