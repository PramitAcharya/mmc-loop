...
function CreatePost() {
  ...
  useEffect(() => {
    if (!loading && !user) navigate({ to: "/auth", search: { next: "/create" }, replace: true });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error || !data) {
      if (imagePath) {
        const { error: rmErr } = await supabase.storage.from("post-images").remove([imagePath]);
    ...
    if (parsed.data.body?.trim() && !anonymous) {
      await syncMentions("post", data.id, parsed.data.body.trim());
    }
    setBusy(false);
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <form onSubmit={handleSubmit} className="surface-panel space-y-5 p-5">
          ...
          {categories.isLoading ? (
            ...
            <div className="rounded-xl border border-destructive/50 bg-destructive/5 p-3">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Please try again before publishing.
              </p>
              <Button
                type="button"
                variant="outline"
              ...
              >
                {categories.isFetching ? "Retrying…" : "Retry"}
              </Button>
            </div>
          ) : (
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">Type @ to mention another student.</p>
        </div>
