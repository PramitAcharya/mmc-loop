...
import { VerificationBadge, AdminBadge } from "@/components/VerificationBadge";
import { fetchMyPostVotes, POST_SELECT, type PublicPost } from "@/lib/community";
...
function ProfilePage() {
  ...
  const profile = useQuery({
    ...
    queryFn: async () => {
      // Escape SQL ILIKE wildcards: fallback usernames (e.g. "member_a1b2c3d4")
      // contain "_", which otherwise matches any character and can resolve to
      // multiple rows, making the profile fail to load.
      const term = username.replace(/[\\%_]/g, "\\$&");
      const { data, error } = await supabase
        .from("public_profiles")
        .select(
          "id, username, display_name, bio, avatar_url, reputation, verification_status, created_at, is_admin, is_moderator",
        )
        .ilike("username", term)
        .maybeSingle();
  ...
  if (!profile.data)
    return (
      <EmptyState
        ...
        description="No profile with that username."
        icon="👤"
      />
    );
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="flex items-center gap-1.5 truncate text-xl font-bold">
              ...
              <AdminBadge isAdmin={p.is_admin} isModerator={p.is_moderator} />
            </h1>
