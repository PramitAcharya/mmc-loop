...
function SettingsPage() {
  ...
  async function saveProfile(e: React.FormEvent) {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ display_name: name || null, bio: bio.trim() || null })
      .eq("id", profile.id);
      ...
      {profile ? (
        <>
          <section aria-labelledby="username-section" className="surface-panel space-y-3 p-5">
            ...
            <p className="text-sm text-muted-foreground">
              You are currently <span className="font-medium text-foreground">@{profile.username}</span>.
              Changing it keeps all of your existing posts, comments and votes.
            </p>
            <UsernamePicker
