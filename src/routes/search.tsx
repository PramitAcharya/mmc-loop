...
function SearchPage() {
  ...
  const people = useQuery({
    ...
    enabled: !!user && hasQuery && tab !== "posts",
    queryFn: async () => {
            ...
            <section aria-labelledby="people-results" className="space-y-3">
              ...
              {!user ? (
                <EmptyState
                  ...
                  description="The member directory is private. Sign in to look up students by username or name."
                  icon="🔒"
                  action={
                    <Button asChild size="sm">
                      <Link to="/auth" search={{ next: "/search" }}>
                        Sign in
                      </Link>
                    </Button>
                  }
              ...
              {!people.isLoading && !!user && peopleList.length === 0 ? (
